...
Point = namedtuple('Point', ['x', 'y'])
 # funkcja wywołująca się po przycisku - losuje 4 punkty z zakresu od -100 do 100
def Losuj4():
    points = [Point(random.randint(-100, 100), random.randint(-100, 100)) for _ in range(4)]
    wynik=main(points, canvas_frame)
    typ_wierzcholki.config(text=wynik)
    okno.geometry("900x700")
 # funkcja wywołująca się po przycisku - pobiera punkty podane przez użytkownika
def PobierzPunkty():
...
otoczka100_label = tk.Label(prawa_kolumna, text="Otoczka 4 losowych punktów\n(zakres: -100 do 100)", justify="left")
otoczka100_label.pack(padx=10, pady=(10, 5))

otoczka100_button = tk.Button(prawa_kolumna, text="Rysuj otoczkę", command=Losuj4)
